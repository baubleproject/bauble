    ...
    return (
        <section {...props} className={cn("", className)}>
            ...
            */}
            ...
            <div className="text-sm font-light mt-3 -tracking-wider">Admin(s)</div>
                ...
                {members?.map(
                    (t) =>
                        t.role === MemberRole.ADMIN && (
                            <div
                                key={t.id}
                                className="px-2 py-3 flex gap-2 items-center bg-zinc-300 dark:bg-zinc-800 rounded-lg"
                            >
                                <div
                                    className="cursor-pointer rounded-full w-7 h-7 bg-center bg-cover bg-no-repeat"
                                    style={{ backgroundImage: `url(${t.profile?.imageUrl})` }}
                                ></div>
                                <div>
                                    <p className="text-sm">
            ...
            <div className="text-sm font-light -tracking-wider">Members(s)</div>
            <div className="w-full min-h-14 flex items-start justify-start py-2 gap-3">
                {members?.map(
                    (t) =>
                        t.role !== MemberRole.ADMIN && (
